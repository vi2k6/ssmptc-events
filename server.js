...
app.get('/api/events/:eventId', (req, res)=>{
  const events = db.readEvents();
  const programs = db.readPrograms();
...
app.get('/api/search', (req, res)=>{
  ...
  const type = (req.query.type || '').toLowerCase(); // upcoming|present|past
    ...
    .map(ev=>{
      ...
      return {
        ...
        programs: evPrograms.filter(p=>{
          ...
          const matchSem = !sem || sem==='' || ['1','2','3','4','5','6'].includes(sem);
...
app.post('/api/register', (req,res)=>{
  ...
  const regs = db.readRegistrations();
  ...
  regs.push(reg);
  db.writeRegistrations(regs);
  res.json({success:true, registration: reg});
});

// Public: my registrations by rollNo
app.get('/api/my-registrations', (req, res)=>{
  ...
  const regs = db.readRegistrations().filter(r=> r.rollNo.toLowerCase()===rollNo.toLowerCase());
  const programs = db.readPrograms();
  const events = db.readEvents();
...
app.post('/api/admin/events', authRequired, (req,res)=>{
  ...
  const events = db.readEvents(); events.push(ev); db.writeEvents(events);
  res.json(ev);
});
...
app.delete('/api/admin/events/:id', authRequired, (req,res)=>{
  const events = db.readEvents().filter(e=> e.id!==req.params.id);
  db.writeEvents(events);
  res.json({success:true});
});

// Admin: CRUD programs
app.get('/api/admin/events/:eventId/programs', authRequired, (req,res)=>{
  const programs = db.readPrograms().filter(p=> p.eventId===req.params.eventId);
  res.json(programs);
});
app.post('/api/admin/events/:eventId/programs', authRequired, (req,res)=>{
  ...
  const programs = db.readPrograms(); programs.push(p); db.writePrograms(programs);
  res.json(p);
});
app.put('/api/admin/programs/:id', authRequired, (req,res)=>{
  const programs = db.readPrograms();
  const idx = programs.findIndex(p=> p.id===req.params.id);
  if(idx<0) return res.status(404).json({error:'Not found'});
  programs[idx] = { ...programs[idx], ...req.body, id: programs[idx].id };
  db.writePrograms(programs);
  res.json(programs[idx]);
});
app.delete('/api/admin/programs/:id', authRequired, (req,res)=>{
  const programs = db.readPrograms().filter(p=> p.id!==req.params.id);
  db.writePrograms(programs);
  res.json({success:true});
});
...
app.get('/api/admin/registrations', authRequired, (req,res)=>{
  res.json(db.readRegistrations());
});
app.put('/api/admin/registrations/:id', authRequired, (req,res)=>{
  const regs = db.readRegistrations();
...
app.delete('/api/admin/registrations/:id', authRequired, (req,res)=>{
  const regs = db.readRegistrations().filter(r=> r.id!==req.params.id);
  db.writeRegistrations(regs);
  res.json({success:true});
});

// Owner: manage admins
app.get('/api/owner/admins', authRequired, ownerRequired, (req,res)=>{
  res.json(db.readAdmins());
});
app.post('/api/owner/admins', authRequired, ownerRequired, (req,res)=>{
  ...
  db.writeAdmins(admins);
  res.json({success:true});
});
app.put('/api/owner/admins/:id', authRequired, ownerRequired, (req,res)=>{
  const admins = db.readAdmins();
...
app.delete('/api/owner/admins/:id', authRequired, ownerRequired, (req,res)=>{
  const admins = db.readAdmins().filter(a=> a.id!==req.params.id);
  db.writeAdmins(admins);
  res.json({success:true});
});
